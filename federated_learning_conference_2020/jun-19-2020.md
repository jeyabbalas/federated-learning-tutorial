...
1. Applied for loan application approval. When user submits request for a loan, the wuery is sent to AI model, the AI model sends prediction and an explanation. Gives both global (population-wise) and local-level (instance-specific) explanations.
